...
class PageRenderService {
  ...
  async renderPages(pdfBuffer, pageNumbers, options = {}) {
    ...
    // Para Poppler se requiere el PDF en un archivo temporal en disco
    ...
    try {
      if (platform === 'linux') {
        ...
        if (popplerAvailable) {
          ...
          try {
            results = await popplerRenderer.render(tempPdfPath, pageNumbers, options);
          } catch (popplerErr) {
            ...
            results = await puppeteerRenderer.render(pdfBuffer, pageNumbers, options);
          }
        } else {
        ...
        try {
          results = await pdfjsRenderer.render(pdfBuffer, pageNumbers, options);
        } catch (pdfjsErr) {
